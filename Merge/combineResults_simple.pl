...
$m=1;
foreach $eachStdHistFile ( glob("../New_Cuts/cen*.root") ) {
    chomp($eachStdHistFile);
	print combineMacro  "toBeAddedFiles->AddLast((TObject *)(new TObjString(\"$eachStdHistFile\"))); \n";
	$m ++;
}
# Modify this path to point to the correct directory containing your files

# Declare the variable cen
my $cen = shift;
my $energy = 7;
foreach my $eachStdHistFile (glob("../New_Cuts/cen${cen}_${energy}GeV/cen${cen}.${energy}GeVAMPT_job*.root")) {
    chomp($eachStdHistFile);
    print combineMacro  "toBeAddedFiles->AddLast((TObject *)(new TObjString(\"$eachStdHistFile\"))); \n";
}
# original
# foreach my $eachStdHistFile (glob("../New_Cuts/new_cen${cen}results_newCuts/cen${cen}.AMPT_simulation_job*.root")) {
#     chomp($eachStdHistFile);
#     print combineMacro  "toBeAddedFiles->AddLast((TObject *)(new TObjString(\"$eachStdHistFile\"))); \n";
# }
# try import energy
# foreach my $eachStdHistFile (glob("../New_Cuts/cen${cen}_${energy}GeV/cen${cen}.${energy}GeVAMPT__job*.root")) {
#     chomp($eachStdHistFile);
#     print combineMacro  "toBeAddedFiles->AddLast((TObject *)(new TObjString(\"$eachStdHistFile\"))); \n";
# }


print combineMacro "   TFile* newfile = new TFile(\"cen${cen}_${energy}GeV.merged.root\",\"RECREATE\"); \n";
